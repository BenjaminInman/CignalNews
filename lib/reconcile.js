...
export function reconcile(facts, opts = {}) {
  ...
  for (const [k, rows] of groups) {
    ...
    if (verified.length) {
      ...
      // Corroboration counts sources that AGREE. A source whose value was
      // rejected is not corroboration — counting it inflates confidence using
      // the very evidence that was thrown out.
      const agreeing = rows.filter((r) => !conflict(v, r));
      out.push({
        ...v,
        corroboration: new Set(agreeing.map((r) => (r.source_name || "?").toLowerCase())).size,
