...
import { resolveImage, localImageFor } from "./images";
...
export async function getNews() {
  ...
  items = await Promise.all(items.map(async (it) => ({ ...it, image: await resolveImage(it) })));
...
const FALLBACK = Array.from({ length: 12 }).map((_, i) => {
  ...
  return { ...item, image: localImageFor(item) };
});
